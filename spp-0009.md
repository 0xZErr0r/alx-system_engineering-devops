...
In Smilepay, transactions between multiple accounts vary as the account type of the sender and recipient accounts determines the transaction type. As such the quota type & quota amount varies for each transaction types (SPP: 0012). Here are the different transaction types in Smilepay: 
...
Transaction ID is an Alphanumeric around 15 digits identification number used to trace transactions and is required by NBE license. Here are the 36 possible alphanumerics for each ~15 digits:  
``` 
    | No. of  | Alpha   | 
    | Possib. | Numeric |
    | ------  | ------- |
...
To calculate the possible number of combinations for a Transaction ID we need to calculate the 36 Possibilities for each 15 digit. But as Txn ID will be randomly generated, a newly generated Txn ID will have to always check if it has been used before. <br />

The Formula:
> 36^15 = 2.210739197 E+23 <br />
> &emsp;&emsp;&emsp; = 221,073,919,720,733,357,899,776 <br /> 
> &emsp;&emsp;&emsp; = 221.1 Sextillion Possibilities 
